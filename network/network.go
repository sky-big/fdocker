...
func configEndpointIpAddressAndRoute(ep *Endpoint, cinfo *types.ContainerInfo) error {
	peerLink, err := netlink.LinkByName(ep.Device.PeerName)
	if err != nil {
		return fmt.Errorf("fail config endpoint: %v", err)
	}

	peerLink.Attrs().MTU = MTU
...
func configPortMapping(ep *Endpoint, cinfo *types.ContainerInfo) error {
...
func Connect(networkName string, cinfo *types.ContainerInfo) (string, error) {
...
func Disconnect(cinfo *types.ContainerInfo) error {
	network, ok := networks[cinfo.NetworkName]
	if !ok {
	...
	err := ipAllocator.Release(network.IpRange, &ipaddr)
	if err != nil {
		return err
	}
	...
	// 删除网络端点
	ep := &Endpoint{
		ID: fmt.Sprintf("%s-%s", cinfo.Id, cinfo.NetworkName),
	}
	if err = drivers[network.Driver].Disconnect(network, ep); err != nil {
		return err
	}
